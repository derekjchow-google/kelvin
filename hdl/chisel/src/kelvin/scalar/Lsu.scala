...
object LsuUOp {
  def apply(i: Int,
    ...
    if (fbus.isDefined) {
      result.addr := Mux(
          cmd.op === LsuOp.FLOAT, fbus.get.addr(i), sbus.addr(i))
      result.data := Mux(
...
class LsuSlot(bytesPerSlot: Int, bytesPerLine: Int) extends Bundle {
  ...
  def loadUpdate(valid: Bool,
    ...
    val lineActive = VecInit((0 until bytesPerSlot).map(i =>
        !store &&     // Don't update if not a store
        (!LsuOp.isVector(op) || !pendingVector) &&  // Has vector data
  ...
  // Updates the slot if it's result is written back to the regfile.
...
object LsuSlot {
  ...
  def fromLsuUOp(uop: LsuUOp, p: Parameters, bytesPerSlot: Int): LsuSlot = {
    // TODO(derekjchow): Unhardcode AXI
...
class LsuV2(p: Parameters) extends Lsu(p) {
  ...
  // TODO(derekjchow): Vector writeback
  if (p.enableRvv) {
    // TODO(derekjchow): Connect this slot in
